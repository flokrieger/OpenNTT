...
module TwiddleGen #(
    ...
    parameter MEMORY_OPTIMIZED = 0,     // if set to 1: less ROM memory is required at the cost of more complex logic and routing
  ...
  if(NTT && MEMORY_OPTIMIZED && INTMUL_LAT+MODRED_LAT > 31)
    $error("Invalid modular multiplier latency for memory optimized version\n");
  ...
  if(NTT_TYPE == "fntt_dit_nr" || NTT_TYPE == "fntt_dit_nr-intt_dit_rn") begin
    if(MEMORY_OPTIMIZED) begin
      if(PE == 1) begin
        logic rst_1DP;
        always_ff @( posedge clk ) rst_1DP <= rst;
        TwiddleGen_DIT_PE1 #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .NR(1), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
        ...
        TwiddleGen_DIT_NR #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                            .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT),  .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH),
                            .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT), .INSTANTIATE_MODMULT(IS_UNIFIED?0:1)) 
                          dit_nr 
      ...
      TwiddleGen_rout_DIT_NR #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                                .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT), .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH),
                                .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT), .INSTANTIATE_MODMULT(IS_UNIFIED?0:1)) 
                              dit_nr 
                              (.clk(clk),.rst(rst),.q(q_internal),.tw_out(tw_out_0),.rom_base_addr(rom_base_addr_internal),.modmul_a(modmult_a_forw),.modmul_b(modmult_b_forw),.modmul_c(modmult_c));
  ...
  if(NTT_TYPE == "fntt_dif_nr") begin
    if(MEMORY_OPTIMIZED)
      TwiddleGen_DIF_NR #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
      ...
      TwiddleGen_rout_DIF_NR #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                                .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT), .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH),
                                .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT)) 
                              dif_nr 
                              (.clk(clk),.rst(rst),.q(q_internal),.rom_base_addr(rom_base_addr_internal),.tw_out(tw_out_0));
  ...
  if(NTT_TYPE == "intt_dit_rn" || NTT_TYPE == "fntt_dit_nr-intt_dit_rn") begin
    if(MEMORY_OPTIMIZED) begin
      if(PE == 1) begin
        logic rst_1DP;
        always_ff @( posedge clk ) rst_1DP <= rst;
        TwiddleGen_DIT_PE1 #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .NR(0), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                                .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT), .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH),
                                .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT), .INSTANTIATE_MODMULT(IS_UNIFIED?0:1)) 
                              dit_rn 
        ...
        TwiddleGen_DIT_RN #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                            .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT), .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH),
                            .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT), .INSTANTIATE_MODMULT(IS_UNIFIED?0:1)) 
                          dit_rn 
      ...
      TwiddleGen_rout_DIT_RN #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                                .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT),  .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH),
                                .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT), .INSTANTIATE_MODMULT(IS_UNIFIED?0:1)) 
                              dit_rn 
                              (.clk(clk),.rst(rst),.q(q_internal),.tw_out(tw_out_tmp),.rom_base_addr(rom_base_addr_internal),.modmul_a(modmult_a_inv),.modmul_b(modmult_b_inv),.modmul_c(modmult_c));
  ...
  if(NTT_TYPE == "intt_dif_rn") begin
    if(MEMORY_OPTIMIZED)
      TwiddleGen_DIF_RN #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                          .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT),
                          .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT)) 
                        dif_rn 
                        (.clk(clk),.rst(rst),.q(q_internal),.rom_base_addr(rom_base_addr_internal),.tw_out(tw_out_0));
    else
      TwiddleGen_rout_DIF_RN #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                                .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT),  .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH), 
                                .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT)) 
                              dif_rn 
                              (.clk(clk),.rst(rst),.q(q_internal),.rom_base_addr(rom_base_addr_internal),.tw_out(tw_out_0));
  ...
  if(NTT_TYPE == "mfntt_dit_nr" || NTT_TYPE == "mfntt_dit_nr-mintt_dif_rn") begin
    if(MEMORY_OPTIMIZED)
      TwiddleGen_Merged_DIT_NR #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                                  .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT),  .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH),
                                  .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT), .INSTANTIATE_MODMULT(IS_UNIFIED?0:1)) 
                                dit_nr 
                                (.clk(clk),.rst(rst),.q(q_internal),.tw_out(tw_out_0),.rom_base_addr(rom_base_addr_internal),.modmul_a(modmult_a_forw),.modmul_b(modmult_b_forw),.modmul_c(modmult_c));
    else
      TwiddleGen_rout_DIT_NR #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .IS_MERGED(1), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                                .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT),  .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH),
                                .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT), .INSTANTIATE_MODMULT(IS_UNIFIED?0:1)) 
                              dit_nr 
                              (.clk(clk),.rst(rst),.q(q_internal),.tw_out(tw_out_0),.rom_base_addr(rom_base_addr_internal),.modmul_a(modmult_a_forw),.modmul_b(modmult_b_forw),.modmul_c(modmult_c));
  ...
  if(NTT_TYPE == "mintt_dif_rn" || NTT_TYPE == "mfntt_dit_nr-mintt_dif_rn") begin
    if(MEMORY_OPTIMIZED)
      TwiddleGen_Merged_DIF_RN #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .ROM_RD_LAT(ROM_RD_LAT), .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                                  .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT),
                                  .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT), .INSTANTIATE_MODMULT(IS_UNIFIED?0:1)) 
                                dif_rn 
                                (.clk(clk),.rst(rst),.q(q_internal),.tw_out(tw_out_tmp),.rom_base_addr(rom_base_addr_internal),.modmul_a(modmult_a_inv),.modmul_b(modmult_b_inv),.modmul_c(modmult_c));
    else
      TwiddleGen_rout_DIF_RN #(.NTT(NTT), .LOGQ(LOGQ), .LOGN(LOGN), .PE(PE), .IS_MERGED(1), .ROM_RD_LAT(ROM_RD_LAT), .MEM_TYPE(MEM_TYPE), .INTMUL_LAT(INTMUL_LAT), 
                                .INTMUL_TYPE(INTMUL_TYPE), .Q_VALUE(Q_VALUE), .WORD_SIZE(WORD_SIZE), .MODRED_LAT(MODRED_LAT),  .ROM_ADDR_WIDTH(ROM_ADDR_WIDTH),
                                .MODRED_TYPE(MODRED_TYPE), .MODRED_L(MODRED_L), .MODRED_COREMUL_LAT(MODRED_COREMUL_LAT), .INSTANTIATE_MODMULT(IS_UNIFIED?0:1)) 
                              dif_rn 
                              (.clk(clk),.rst(rst),.q(q_internal),.tw_out(tw_out_tmp),.rom_base_addr(rom_base_addr_internal),.modmul_a(modmult_a_inv),.modmul_b(modmult_b_inv),.modmul_c(modmult_c));
      ...
      for(genvar g=0; g < PE; g=g+1) begin
        if(NTT) begin
          ...
          ComplexMultiplier cmul (
            .clk(clk),
            .start(),
            .a(forward_reg ? modmult_a_forw[g] : modmult_a_inv[g]),
            .b(forward_reg ? modmult_b_forw[g] : modmult_b_inv[g]),
            .c(modmult_c[g]),
            .done()
          );
        end
