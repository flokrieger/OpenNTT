...
// Twiddle factor generation for a single PE using RoutOpt DIF NR transformations
module TwiddleGen_rout_DIF_NR_peModule #(
  parameter NTT = 1,
  ...
  localparam string rom_filename = NTT ? $sformatf("%s/tool/RomContent/twiddle_mem_rout_DIF_NR_n%0d_pe%0d_lat%0d%s%0d.mem",    open_ntt_pkg::OPEN_NTT_PATH, 1<<LOGN, PE, MODMUL_LAT, MODRED_TYPE=="default"?"_MontRed_":"_", PE_NUMBER) : 
                                         $sformatf("%s/tool/RomContent/twiddle_mem_rout_DIF_NR_n%0d_pe%0d_lat%0d_FFT_%0d.mem", open_ntt_pkg::OPEN_NTT_PATH, 1<<LOGN, PE, MODMUL_LAT, PE_NUMBER);
  ...
  logic [$clog2(N/2/PE):0] nr_rep, nr_rep_3DP;
  logic rep;
  ...
  // control signals:
  logic stage_repeats;
  assign stage_repeats = s_i > $clog2(PE);
  ...
  assign rep = stage_repeats && s_i < LOGN-2 && (t & (N/2/PE-1 >> (s_i-$clog2(PE)))) == {$clog2(N/2/PE){1'd0}} && i_i != {$clog2(N/2/PE){1'd0}};

  always_ff @( posedge clk ) begin
    if (rst_internal)
      nr_rep <= N/2/PE;
    else if (stage_done_initial && s_i > $clog2(PE))
      nr_rep <= nr_rep >> 1;
  end
  shiftreg #(.DELAY(ROM_RD_LAT+1), .LOGQ($clog2(N/2/PE)+1)) nr_rep_delay (
    .clk(clk),
    .data_in(nr_rep),
    .data_out(nr_rep_3DP)
  );
  ...
  assign omega_c_sel = MODMUL_LAT % 2 == 0 ? 1'd0 : (1<<$clog2(MODMUL_LAT+1) == MODMUL_LAT+1) && nr_rep_3DP == MODMUL_LAT+1 ? ~i_rom[0] : i_rom[0];
