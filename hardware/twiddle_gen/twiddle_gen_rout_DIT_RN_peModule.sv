...
// Twiddle factor generation for a single PE using RoutOpt DIT RN transformations
module TwiddleGen_rout_DIT_RN_peModule #(
  parameter NTT = 1,
  ...
  localparam string rom_filename = NTT ? $sformatf("%s/tool/RomContent/twiddle_mem_rout_DIT_RN_n%0d_pe%0d_lat%0d%s%0d.mem",    open_ntt_pkg::OPEN_NTT_PATH, 1<<LOGN, PE, MODMUL_LAT, MODRED_TYPE=="default"?"_MontRed_":"_", PE_NUMBER) : 
                                         $sformatf("%s/tool/RomContent/twiddle_mem_rout_DIT_RN_n%0d_pe%0d_lat%0d_FFT_%0d.mem", open_ntt_pkg::OPEN_NTT_PATH, 1<<LOGN, PE, MODMUL_LAT, PE_NUMBER);
  ...
  assign is_omega_c_addr = i_i == {$clog2(N/2/PE){1'd1}};
  ...
  assign update_mult_adv = i_i != {($clog2(N/2/PE)){1'd1}} && (tmp_i == {($clog2(N/2/PE)+1){1'd0}} || (i_i & (tmp_i-1)) == ({($clog2(N/2/PE)){1'd1}} & (tmp_i-1)));
