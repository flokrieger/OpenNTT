...
def ROM_content_to_mem(twiddles,N,nr_pu,modmul_lat,dec_method,path="../../hardware/twiddle_gen/twiddle_mem_FFT/"):
  nr_brams = len(twiddles)
  for bram in range(nr_brams):
    with open(path + f"twiddle_mem_{dec_method}_n{N}_pe{nr_pu}_lat{modmul_lat}_FFT_{bram}.mem","w") as f:
      f.write("@0000\n")
      for i,e in enumerate(twiddles[bram]):
         t = getUint64FromDouble(e.real) << 64
         t = t | getUint64FromDouble(e.imag)
         f.write(hex(t)[2:]+"\n")


def rom_content_DIF_NR(N,nr_pu,modmul_lat,omega):
  initial_fill = [[0]*modmul_lat for _ in range(nr_pu)]

  for i in range(modmul_lat):
      for j in range(nr_pu):
          if nr_pu == 1:
            initial_fill[j][i] = omega**((i>>1) + (i & 1)*N//4)
          else:
            initial_fill[j][i] = omega**(i+j*(N//2//nr_pu))
  
  omega_c_init = (omega**modmul_lat) if nr_pu > 1 else (omega**(modmul_lat//2))
  return initial_fill, omega_c_init

def rom_content_DIF_RN(N,nr_pu,modmul_lat,omega):
  initial_fill = [[0]*modmul_lat for _ in range(nr_pu)]
  ...
  for i in range(modmul_lat):
      for j in range(nr_pu):
          if nr_pu == 1:
            ...
            initial_fill[j][i] = omega**((i>>1)*nr_pu+(i&1)*N//4+j_br)

  omega_c_init = (omega**(modmul_lat*nr_pu//2))
  return initial_fill, omega_c_init

def rom_content_DIT_NR(N,nr_pu,modmul_lat,omega):
  initial_fill = []

  initial_fill += [1]
  for i in range(0,clog2(N)-1):
    initial_fill += [omega**((N//4 >> i))]
    tmp = i-(clog2(N)-clog2(modmul_lat)-1)
    if tmp > 0 and nr_pu == 1:
      for j in range(1,(2<<(tmp))-1):
        x = (omega**((N//4 >> i) + ((N//4 >> (i)))*j))
        initial_fill += [x]

  return initial_fill

def rom_content_DIT_RN(N,nr_pu,modmul_lat,omega):
  initial_fill = []
...
def rom_content_Merged_DIF_RN(N,nr_pu,modmul_lat,omega):
  initial_fill = [[0]*modmul_lat for _ in range(nr_pu)]
  ...
  for i in range(modmul_lat):
      for j in range(nr_pu):
          ...
          initial_fill[j][i] = initial_fill[j][i]**2
          if j == 0:
            initial_fill[j][i] = (initial_fill[j][i] * omega)
  omega_c_init = (omega**(modmul_lat*nr_pu//2))
  omega_c_init = omega_c_init**2
...
def rom_content_Merged_DIT_NR(N,nr_pu,modmul_lat,omega):
  initial_fill = [1, omega**(N//2)]
  ...
  for i in range(0,clog2(N)-1):
    ...
    if tmp > 0 and nr_pu != 1:
      for j in range(1,(2<<(tmp))-2):
        ...
        x = (omega**((N//4 >> i) + 2*((magic_value*nr_pu*j)>>(tmp-1))))
        initial_fill += [x]

  return initial_fill


def TW_DIF_NR(omega,N,nr_pu,modmul_lat):
    ...
    initial_fill, omega_c_0 = rom_content_DIF_NR(N,nr_pu,modmul_lat,omega)
    omega_i = [0]*nr_pu
        ...
        for i in range(N//2//nr_pu):
            for p in range(nr_pu):
                ...
                if i < modmul_lat and s == 0:
                  tw = initial_fill[0][i] if p == 0 else complexMult_double_nosubnormal(initial_fill[0][i], omega_i[p])
                elif i < modmul_lat or repeat:
                    tw_tmp = fifo[s%2].queue[0]
                    if p == nr_pu-1 and s != clog2(N)-1:
                      r = fifo[s%2].get()
                      fifo[s%2].put(r) 
                    tw = tw_tmp if p == 0 else complexMult_double_nosubnormal(tw_tmp , omega_i[p])

                else:
                  tw = complexMult_double_nosubnormal(produced_tw[p][-modmul_lat] , omega_c_0)
...
def TW_DIF_RN(omega,N,nr_pu,modmul_lat):
    ...
    ############# precomputed rom content ###############
    initial_fill, omega_c_0 = rom_content_DIF_RN(N,nr_pu,modmul_lat,omega)
    omega_i = [0]*nr_pu
    for i in range(1,nr_pu):
       omega_i[i] = initial_fill[i][0]
    #####################################################
...
def TW_DIT_NR(omega,N,nr_pu,modmul_lat):
    if N//2//nr_pu <= modmul_lat:
        print("Warning: use stored twiddle factors for this configuration")
        return None
    if modmul_lat != 4 and modmul_lat != 8 and modmul_lat != 16  and modmul_lat != 32:
    ...
    ############# precomputed rom content ###############
    initial_fill = rom_content_DIT_NR(N,nr_pu,modmul_lat,omega)
        ...
        for p in range(nr_pu):
          ...
          if nr_pu == 1:
            ...
            if i < (N//2//nr_pu >> s):
              ...
              produced_tw[p] += [complexMult_double_nosubnormal(produced_tw[p][-modmul_lat] , omega_c_0)]             
            else:
            ...
            if i < modmul_lat:
              if p >= nr_pu//2 and s > 0:
                t = complexMult_double_nosubnormal(produced_tw[p-nr_pu//2][-1] , omega_i)
              elif p < nr_pu//2:
                t = fifo[p].queue[0]
                fifo[p].get()
              else:
...
def TW_DIT_RN(omega,N,nr_pu,modmul_lat):
    if N//2//nr_pu <= modmul_lat:
        print("Warning: use stored twiddle factors for this configuration")
        return None
    if modmul_lat != 4 and modmul_lat != 8 and modmul_lat != 16 and modmul_lat != 32:
    ...
    ############# precomputed rom content ###############
    initial_fill = rom_content_DIT_RN(N,nr_pu,modmul_lat,omega)
    omega_c_0 = initial_fill[0]
    initial_fill = initial_fill[1:]
    #####################################################
      ...
      for i in range(N//2//nr_pu):
        for p in range(nr_pu):
          ...
          if nr_pu == 1:
            ...
            if i < modmul_lat:
              t = fifo.queue[0]
              t = complexMult_double_nosubnormal(t , omega_i[p])
              if p == nr_pu-1:
                fifo.get()
              produced_tw[p] += [t]
            elif update_mult:
        ...
        if nr_pu > 1 and s < clog2(N)-1 and added_fifo < modmul_lat:
            ...
            if s < special_stage-1:
              ...
              if (not (magic != 0 and (i & magic) == 0)) or (magic == 0):
                t1 = complexMult_double_nosubnormal(new_omega , t)
                added_fifo += 1
                fifo.put(t1)
...
def TW_Merged_DIF_RN(omega,N,nr_pu,modmul_lat):
    ...
    ############# precomputed rom content ###############
    initial_fill, omega_c_0 = rom_content_Merged_DIF_RN(N,nr_pu,modmul_lat,omega)
    omega_i = [0]*nr_pu
    for i in range(1,nr_pu):
       omega_i[i] = initial_fill[i][0]
    #####################################################
            ...
            for p in range(nr_pu):
                ...
                if i < modmul_lat and s == 0:
                  ...
                  if p == nr_pu-1 and s != clog2(N)-1:
                    r = fifo[s%2].get()
                    fifo[s%2].put(r) 
                  tw = complexMult_double_nosubnormal(tw_tmp,pivot) if p == 0 else complexMult_double_nosubnormal(tw_tmp , omega_i[p])
...
def TW_Merged_DIT_NR(omega,N,nr_pu,modmul_lat):
    if N//2//nr_pu <= modmul_lat:
        print("Warning: use stored twiddle factors for this configuration")
        return None
    if modmul_lat != 4 and modmul_lat != 8 and modmul_lat != 16 and modmul_lat != 32:
    ...
    ############# precomputed rom content ###############
    initial_fill = rom_content_Merged_DIT_NR(N,nr_pu,modmul_lat,omega)
    omega_c_0 = initial_fill[0]
    initial_fill = initial_fill[1:]
    omega_i = [1] * nr_pu
    #####################################################
        ...
        for p in range(nr_pu):
          ...
          if i < modmul_lat:
            if p == 0:
              ...
              t = complexMult_double_nosubnormal(produced_tw[0][-1] , omega_i[p])
            produced_tw[p] += [t]
          elif update_mult:
            produced_tw[p] += [complexMult_double_nosubnormal(produced_tw[p][-modmul_lat] , omega_c_0)] 
          else:
